...
the CSV parsing functionality at its core without involving the entire bot infrastructure.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('csv_test_results.log')
    ]
)
        ...
        for i, row in enumerate(reader):
            ...
            if len(row) >= 6:
                try:
                    ...
                    if timestamp_str:
                        try:
                            # Try to parse timestamp (attempt multiple formats)
                            timestamp = None
