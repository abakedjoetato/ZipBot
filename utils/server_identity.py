...
def identify_server(server_id: str, hostname: Optional[str] = None, 
    ...
    """Identify server numeric ID"""
    # Check if we already have a numeric ID
    if str(server_id).isdigit():
        return server_id, True

    # Try to find in known mappings first
    if server_id in KNOWN_SERVERS:
        return KNOWN_SERVERS[server_id], True

    # For UUID format, extract any numeric portion
    if '-' in server_id:
        # Keep original numeric ID if we have it
        if hasattr(identify_server, 'original_ids') and server_id in identify_server.original_ids:
            return identify_server.original_ids[server_id], True
